...
declare -a TEST_FILES=(
    ...
    "test_scope_end.cpp"
    "test_ring_buffers.cpp"
    "test_shared_header.cpp"
    "test_dep_list_pool.cpp"
)
...
run_test() {
    ...
    if [[ -n "$results_line" ]]; then
        ...
        if [[ "$failed" -eq 0 ]]; then
            TEST_RESULTS+=("${GREEN}✓${NC} $test_name: ${GREEN}$passed passed${NC}")
        else
            TEST_RESULTS+=("${RED}✗${NC} $test_name: ${GREEN}$passed passed${NC}, ${RED}$failed failed${NC}")
        fi
        ...
        TEST_RESULTS+=("${RED}✗${NC} $test_name: ${RED}No results found${NC}")
        if $VERBOSE; then
            echo "$output"
        fi
    fi
